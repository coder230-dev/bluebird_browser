...
function addToRecentCache(item) {
	// Remove duplicates
	window.cachedHistoryRecent = window.cachedHistoryRecent.filter(i => i.url !== item.url);
	// Add to front
	window.cachedHistoryRecent.unshift({
		url: item.url,
		title: item.title,
		// Don't store favicon base64 - just store the URL
		faviconUrl: item.faviconUrl
	});
	// Trim
	if (window.cachedHistoryRecent.length > MAX_RECENT_CACHE) {
		window.cachedHistoryRecent.length = MAX_RECENT_CACHE; // faster than shift()
	}
}
	...
	return new Promise((resolve, reject) => {
		...
		request.onupgradeneeded = (event) => {
			...
			if (!db.objectStoreNames.contains('zoomPercent')) {
				db.createObjectStore('zoomPercent', { keyPath: 'url' });
			}
		};
...
function openPrivateTab(url = 'pages/new-tab/index.html') {
	createTab(url, { private: true, insertAfterActive: true });
}

async function clearBrowsingData() {
	if (!window.electronAPI?.clearBrowsingData) return false;
	return await window.electronAPI.clearBrowsingData();
}
...
function formatReleaseNotes(releaseNotes) {
	...
	if (Array.isArray(releaseNotes)) {
		return releaseNotes.map(item => `<p>${String(item)}</p>`).join('');
	}
...
function showPopup(title, contentHtml, actions = []) {
	...
	`;
	const popup = createOverlayPopup(html, '560px', 'auto');
	const actionsContainer = popup.querySelector('.popup-actions');
	if (!actionsContainer) return popup;
	actions.forEach(action => {
		const button = document.createElement('button');
		button.textContent = action.label || 'Close';
		button.className = action.primary ? 'popup-action-btn primary' : 'popup-action-btn';
		button.onclick = () => {
			if (action.onClick) action.onClick();
			popup.classList.remove('open');
			setTimeout(() => {
				const backdrop = document.getElementById('backdrop-for-pop');
...
function setUpdateButtonState(state, info = null) {
	updateState = state;
	updateInfo = info;
	if (!updateBtn) return;
	if (state === 'idle' || state === 'error') {
		...
		updateBtn.title = 'App updates';
		return;
	}
	updateBtn.classList.remove('hidden');
	updateBtn.setAttribute('data-state', state);
	const iconName = state === 'downloaded' ? 'restart_alt' : 'system_update';
	updateBtn.innerHTML = `<i class="material-symbols-rounded">${iconName}</i><span class="update-badge"></span>`;
	updateBtn.title = state === 'downloaded' ? 'Restart to install update' : 'Update available';
}
...
function showUpdateDetails() {
	...
	const versionLabel = updateInfo.version ? `Version ${updateInfo.version}` : 'New version available';
	const notesHtml = formatReleaseNotes(updateInfo.releaseNotes);
	const statusCopy = updateState === 'downloaded'
		? 'The update has finished downloading and is ready to install.'
		: 'The update is downloading in the background. Once completed, the button will become Restart to Update.';
	showPopup(`${versionLabel} is available`, `
		<div class="update-popup-copy">
			<p>${statusCopy}</p>
			<div class="update-notes-wrapper">
				${notesHtml}
			</div>
		</div>
	`, [
		{
			label: updateState === 'downloaded' ? 'Restart to Update' : 'Close',
			primary: updateState === 'downloaded',
			onClick: () => {
...
window.electronAPI?.onUpdateAvailable((data) => {
	setUpdateButtonState('available', data);
});

window.electronAPI?.onUpdateDownloaded((data) => {
	setUpdateButtonState('downloaded', data);
});

window.electronAPI?.onUpdateError((error) => {
	console.warn('Update error:', error?.message || error);
	if (updateBtn) {
		updateBtn.title = `Update error: ${error?.message || 'unknown'}`;
	}
});
...
window.electronAPI?.onUpdateDownloadProgress((progress) => {
	if (updateState === 'available' && updateBtn) {
		const percent = Math.round(progress?.percent || 0);
		updateBtn.title = `Downloading update… ${percent}%`;
	}
});
...
document.addEventListener('DOMContentLoaded', async function () {
	if (!await loadSetting('defaultZoom')) {
		updateSetting('defaultZoom', 100)
	}
...
async function savePermission(url, permission, decision) {
	...
	const key = `${normalizedUrl}:${permission}`;
	const legacyKey = `${url}:${permission}`;
...
async function deletePermission(url, permission) {
	const db = await openDB();
	const tx = db.transaction('permissions', 'readwrite');
	const store = tx.objectStore('permissions');
	const normalizedUrl = getBaseURL(url);
...
function getPermissionLookupKeys(permission) {
	const mapping = {
		media: ['media', 'camera', 'microphone'],
		camera: ['camera', 'media'],
		microphone: ['microphone', 'media'],
		audioCapture: ['audioCapture', 'media'],
		midi: ['midi', 'midiSysex'],
		midiSysex: ['midiSysex', 'midi'],
...
function resolvePermissionDecision(permission, decisions) {
	if (permission === 'media') {
		const mediaDecision = decisions.media;
		const cameraDecision = decisions.camera;
		const microphoneDecision = decisions.microphone;
		if (cameraDecision === 'deny' || microphoneDecision === 'deny') return 'deny';
		if (cameraDecision === 'allow' && microphoneDecision === 'allow') return 'allow';
		return mediaDecision;
	}
	if (permission === 'audioCapture') {
		return decisions.audioCapture || decisions.media;
	}
	if (permission === 'midi') {
		return decisions.midi || decisions.midiSysex;
	}
	if (permission === 'midiSysex') {
		return decisions.midiSysex || decisions.midi;
	}
	return decisions[permission] || undefined;
}
...
async function loadPermission(url, permission) {
	...
	const normalizedUrl = getBaseURL(url);
	const decisions = {};
	for (const key of getPermissionLookupKeys(permission)) {
		const fullKey = `${normalizedUrl}:${key}`;
		const res = await promisifyRequest(store.get(fullKey));
		if (res?.decision) decisions[key] = res.decision;
	}
	if (Object.keys(decisions).length === 0) {
		const legacyKey = `${url}:${permission}`;
		const legacyRes = await promisifyRequest(store.get(legacyKey));
		return legacyRes?.decision;
	}
	return resolvePermissionDecision(permission, decisions);
}

async function loadAllSitePermissions(url) {
	const db = await openDB();
	const tx = db.transaction('permissions', 'readonly');
	const store = tx.objectStore('permissions');
	...
	return (all || []).reduce((acc, entry) => {
		if (!entry?.url || typeof entry.url !== 'string') return acc;
		if (entry.url.startsWith(`${normalizedUrl}:`)) {
			const permissionKey = entry.url.slice(normalizedUrl.length + 1);
			acc[permissionKey] = entry.decision;
		}
		return acc;
	}, {});
}

async function applySitePermissionsToWebview(webview, url) {
	if (!webview || !url) return {};
	const permissions = await loadAllSitePermissions(url);
	webview.dataset.sitePermissions = JSON.stringify(permissions);
	webview.dataset.siteOrigin = getBaseURL(url);
	return permissions;
}

async function setSitePermission(siteUrl, permission, decision) {
...
async function saveZoomFactor(url, factor) {
	if (!url || typeof factor !== 'number') return;
	await dbSet('zoomPercent', { url: getBaseURL(url), factor });
}

async function loadZoomFactor(url) {
	if (!url) return undefined;
	const record = await dbGet('zoomPercent', getBaseURL(url));
	return typeof record?.factor === 'number' ? record.factor : undefined;
}
...
window.addEventListener("message", (event) => {
	if (event.origin !== window.origin && event.origin !== 'null' && event.origin !== '') return;

	if (event.data.updateSettings) {
		if (typeof event.data.key !== 'undefined') {
			window.api?.settings?.save({ [event.data.key]: event.data.value }).catch(() => { });
		}
		try {
	...
	} else if (event.data.type === 'add-bookmark') {
		const bookmark = event.data.bookmark;
		if (bookmark?.url) {
			addBookmark(activeProfile || 'Default', bookmark, false);
		}
...
function sendMediaListToSidebar() {
	const mediaTabs = tabs.filter(tab => tab.isPlaying).map(tab => ({
		tabId: tab.id,
		title: tab.title,
		url: tab.url,
		favicon: tab.favicon,
...
window.electronAPI?.onSwitchProfile?.(async (profileName) => {
	...
	if (!localStorage.getItem(`firstTime-${activeProfile}`)) {
		...
		openSidebarApp('pages/settings/index.html?welcome=true', '👋 Welcome to Bluebird Browser!', true)
	}
...
setTimeout(function () {
	...
	if (searchModeBtn) {
		searchModeBtn.onclick = () => {
			createContextMenu([
				{ icType: 'GF', icon: 'bookmarks', name: 'Bookmarks', category: 'Set Search Mode', function: () => { startSearchMode('bookmark') } },
				// { icType: 'GF', icon: 'history', name: 'History', category: 'Set Search Mode', function: () => { startSearchMode('history') } }
			], searchModeBtn)
		}
	}
	...
	if (btnSettings) btnSettings.onclick = () => { openSidebarApp('pages/settings/index.html', 'Settings', false) };
	...
	if (btnPrivacy) btnPrivacy.onclick = () => {
		showPopup('Privacy Controls', `
			<p>Open a private tab or clear browsing data for this session.</p>
		`, [
			{
				label: 'New Private Tab',
				primary: true,
				onClick: () => openPrivateTab('pages/new-tab/index.html')
			},
			{
				...
				onClick: async () => {
					...
					if (result) {
						displayNotification('Browsing data cleared.', 'privacy_tip');
					} else {
						displayNotification('Could not clear browsing data.', 'warning');
					}
				}
			}
		]);
...
async function runContextAction(type, payload = {}) {
	...
	const actions = {
		...
		toggleFullscreen,
		openSettings: () => openSidebarApp('pages/settings/index.html', 'Settings', false),
		...
		searchOpenBookmark: () => { startSearchMode('bookmark') },
...
document.getElementById('viewWebsiteInfo').addEventListener('click', async function () {
	...
	items.push(
		{
			icon: 'security', icType: 'GF', name: 'View All Permissions', category: 'Security', function: () => { openSidebarApp(`pages/settings/managePermissions.html?url=${getBaseURL(getActive().url)}`, 'Manage Permissions', true) }
		},
...
function showTabLeftInfo(info) {
	if (!info || !info.tabElement) return;
	setTimeout(() => {
		const active = getActive();
		const existing = document.getElementById("active-overflow-tag");

		if (!active) return;
		const visibility = info.tabElement?.dataset?.visible;
		if (visibility === "visible") {
			if (existing) existing.remove();
			return;
		}

		if (active.id !== info.id || visibility !== "hidden") {
			return;
		}
...
function createTab(url, opts = {}) {
	...
	const finalUA = isInternal ? `${baseUA} BluebirdBrowser 1.2.0` : baseUA;
	wv.setAttribute('useragent', finalUA);
	...
	const tab = {
		...
		listeners: [] // Store all event listeners for cleanup
	};
	...
	// Insert into tabs array
	tabs.splice(insertIndex, 0, tab);

	// Persist open tabs (skip private)
	...
	// Insert webview into DOM
	if (views) {
		const referenceView = views.children[insertIndex];
		if (referenceView) {
	...
	const domReadyHandler = () => {
		...
		wv.executeJavaScript(`
            window.alert = (msg) => window.electronAPI.customAlert(msg);
            window.confirm = (msg) => window.electronAPI.customConfirm(msg);
            window.prompt = (msg, def = "") => window.electronAPI.customPrompt(msg, def);
        `);

		// Pass profile to new tab page
		if (url === 'pages/new-tab/index.html') {
			wv.executeJavaScript(
				`localStorage.setItem('currentProfile', '${activeProfile}');`
			);
		...
		// Register webview and its partition session for permission handling
		try {
			const wcId = wv.getWebContentsId();
			window.electronAPI?.registerWebview?.(wcId);
			const partition = wv.getAttribute('partition');
			if (partition) window.electronAPI?.registerPartition?.(partition);
		} catch (err) {
			console.warn('getWebContentsId failed:', err);
		}
	};
	wv.addEventListener("dom-ready", domReadyHandler);
	tab.listeners.push({ event: "dom-ready", handler: domReadyHandler });
	...
	const stopLoadingHandler = () => {
		...
		const entry = {
			url: wv.getURL(),
			title: wv.getTitle() || tab.title || wv.getURL(),
	...
	// NAVIGATION EVENTS (merged logic)
	const handleNav = (e) => {
		...
		// Internal UA switch
		const internalNow = tab.url.startsWith('file://');
		wv.setAttribute(
			'useragent',
			internalNow ? `${baseUA} BluebirdBrowser 1.2.0` : baseUA
		);

		// Preload toggle
		wv.setAttribute('preload', internalNow ? 'preload.js' : 'preload.js');
		...
		if (id === activeTab) address.blur();
	};

	wv.addEventListener('did-start-navigation', handleNav);
	tab.listeners.push({ event: 'did-start-navigation', handler: handleNav });

	wv.addEventListener('did-navigate', handleNav);
	tab.listeners.push({ event: 'did-navigate', handler: handleNav });

	wv.addEventListener('did-navigate-in-page', handleNav);
	tab.listeners.push({ event: 'did-navigate-in-page', handler: handleNav });

	// DID FINISH LOAD
	const finishLoadHandler = async () => {
		const active = getActive();
		if (active) showTabLeftInfo(active);
		...
		// Screenshot - defer to reduce memory usage
		setTimeout(() => {
			wv.capturePage()
				.then(image => {
					// Only store if tab is still active and not too many
					if (tabs.find(t => t.id === id) && tabs.length < 10) {
						tab.screenshot = image.toDataURL();
					}
				})
				.catch(err => {
					console.warn('Screenshot capture failed for tab:', tab.shortId, err);
				});
		}, 2000); // Delay to reduce CPU usage

		// Zoom
		const savedZoom = await loadZoomFactor(wv.getURL()) || loadSetting('defaultZoom');
	...
	tab.listeners.push({ event: 'did-finish-load', handler: finishLoadHandler });

	// TITLE UPDATED
	const titleUpdatedHandler = (e) => {
		tab.title = e.title || tab.url;
		updateTabElement(tab);
	};
	wv.addEventListener('page-title-updated', titleUpdatedHandler);
	tab.listeners.push({ event: 'page-title-updated', handler: titleUpdatedHandler });

	// FAVICON UPDATED
	const faviconUpdatedHandler = (e) => {
		tab.favicon = e.favicons?.[0] || null;
		updateTabElement(tab);
	};
	wv.addEventListener('page-favicon-updated', faviconUpdatedHandler);
	tab.listeners.push({ event: 'page-favicon-updated', handler: faviconUpdatedHandler });
	...
	wv.addEventListener('media-started-playing', mediaStartedHandler);
	tab.listeners.push({ event: 'media-started-playing', handler: mediaStartedHandler });
	...
	wv.addEventListener('media-paused', mediaPausedHandler);
	tab.listeners.push({ event: 'media-paused', handler: mediaPausedHandler });

	// CLICK → remove context menus
	const clickHandler = () => removeContextMenus();
	wv.addEventListener('click', clickHandler);
	tab.listeners.push({ event: 'click', handler: clickHandler });

	// FIND EVENTS
	attachFindEvents(wv, tab);

	// HISTORY STACK
	tabHistory[shortId] = { stack: [], index: -1 };
...
function loadErrorPage(webview, code, desc = "") {
	const html = `
        <html>
        <head>
            <style>
                ...
                .error-box {
                    ...
                    padding: 20px;
                    border-radius: 12px;
                    background: var(--scheme-2);
                    border: 1px solid var(--scheme-9);
                }
                h1 { margin: 0 0 10px; }
                p { opacity: 0.8; }
            </style>
        ...
        <body>
            <div class="error-box">
                <h1>Error ${code}</h1>
                <p>${desc || "This page could not be loaded."}</p>
...
function attachHistoryListeners(webview, shortId, info) {
	tabHistory[shortId]._listeners = []; // store refs
	...
	const record = () => {
		...
		// Cap history stack to prevent infinite growth
		if (h.stack.length > 50) h.stack.shift(); // keep last 50 only
		h.stack.push(webview.getURL());
		h.index = h.stack.length - 1;
	};

	webview.addEventListener('did-navigate', record);
	tabHistory[shortId]._listeners.push({ event: 'did-navigate', fn: record });

	webview.addEventListener('did-navigate-in-page', record);
	tabHistory[shortId]._listeners.push({ event: 'did-navigate-in-page', fn: record });
}
...
async function closeTab(tabId) {
	...
	if (webview) {
		...
		try { webview.clearHistory(); } catch { }
		try { webview.remove(); } catch { }
...
window.reloadActiveWebview = function () {
	const active = getActive();
...
function openFindPopup(elementClicked = null, x = 0, y = 0) {
	...
	if (nextBtn) nextBtn.onclick = () => doFind(true);
	if (prevBtn) prevBtn.onclick = () => doFind(false);
...
function attachFindEvents(wv, tab) {
	const foundHandler = (e) => {
		const { activeMatchOrdinal, matches } = e.result;

		if (matches == 0) {
			if (text === '') {
				a.webview.stopFindInPage("clearSelection");
			}
		}
	...
	wv.addEventListener("found-in-page", foundHandler);
	if (tab?.listeners) tab.listeners.push({ event: "found-in-page", handler: foundHandler });
}
...
function updateNavButtons(webview) {
	...
	try {
		...
		console.warn('Unable to update back button state:', err);
	}
	try {
		...
		console.warn('Unable to update forward button state:', err);
	}
}
...
async function showZoomControls(webview, zoomSize) {
	...
	saveZoomFactor(webview.getURL(), currentZoom).catch(() => { });
...
async function zoomIn() {
	...
	await a.webview.setZoomFactor(next);
	await saveZoomFactor(a.webview.getURL(), next).catch(() => { });
...
async function zoomReset() {
	const a = getActive();
	if (!a?.webview) return;
	const defaultFactor = (await loadSetting('website.dZoomValue') || 100) / 100;
	await a.webview.setZoomFactor(defaultFactor);
	await saveZoomFactor(a.webview.getURL(), defaultFactor).catch(() => { });
...
async function openMainContextMenu(elementClicked = moreBtn, x = 0, y = 0) {
	...
	const items = [
		...
		},
		{
			...
			submenu: [
				{
					icon: 'collections_bookmark', icType: 'GF', name: 'Manage & View All Bookmarks', category: "Manage",
		...
		{ icon: 'settings', icType: 'GF', name: 'Settings', shortcut: 'Ctrl+,', category: 'App', function: () => openSidebarApp('pages/settings/index.html', 'Settings', false) },
	];
...
function startSearchMode(mode) {
	let viewWebsiteInfo = document.getElementById('viewWebsiteInfo');
...
async function buildSuggestions(queryRaw) {
	...
	// -----------------------------------------
	// 1. EXCLUSIVE SEARCH MODES (bookmark / history)
	// -----------------------------------------

	if (searchMode === 'bookmark') {
		const bookmarks = getBookmarksProfile(activeProfile || 'Default') || [];
		const results = [];

		for (const b of bookmarks) {
			if (!b || !b.url) continue;
			const title = (b.title || b.url).toString();
			if (title.toLowerCase().includes(lower) || b.url.toLowerCase().includes(lower)) {
				results.push({ text: b.url, label: title, type: 'bookmark', sub: b.url });
			}
		}

		// If bookmark mode found results → return ONLY those
		if (results.length > 0) return results;
		// Otherwise fall through to full omnibox
	}

	if (searchMode === 'history') {
		const history = await getHistoryProfile(100) || [];
		const results = [];

		for (const h of history) {
			if (!h || !h.url) continue;
			const title = (h.title || h.url).toString();
			if (title.toLowerCase().includes(lower) || h.url.toLowerCase().includes(lower)) {
				results.push({ text: h.url, label: title, type: 'history', sub: h.url });
			}
		}

		// If history mode found results → return ONLY those
		if (results.length > 0) return results;
		// Otherwise fall through to full omnibox
	}

	// -----------------------------------------
	// 2. FULL OMNIBOX (fallback or no searchMode)
	// -----------------------------------------

	return await buildNormalSuggestions(q, lower);
}

async function buildNormalSuggestions(q, lower) {
	const results = [];
...
if (address) {
	address.addEventListener('focus', () => {
		...
		searchModeBtn.style.display = 'flex';
	});
	...
	address.addEventListener("blur", function (e) {
		...
		// If focus moved to the searchModeBtn → keep it visible
		if (next === searchModeBtn) return;
...
async function openSidebarApp(url, title, iframe = false) {
  ...
  `;
	...
	const sidebarWidth = typeof savedWidth === 'number' ? `${savedWidth}px` : '550px';
	...
	setTimeout(function () {
		...
		if (!iframe) {
			web.setAttribute('preload', 'preload.js');
			web.setAttribute('allowpopups', 'true');
			web.setAttribute('webpreferences', 'contextIsolation=yes, nativeWindowOpen=yes');
		}
		...
		// Store reference to sidebar iframe/webview for message passing
		...
		if (!iframe) {
			web.addEventListener('ipc-message', (event) => {
				...
				if (event.channel === 'settings-update') {
					if (payload.key) {
						window.api?.settings?.save({ [payload.key]: payload.value }).catch(() => { });
					}
					loadBrowserSettings();
				} else if (event.channel === 'add-bookmark') {
